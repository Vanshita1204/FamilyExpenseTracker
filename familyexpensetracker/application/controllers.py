...
def login():
    if request.method=='POST':
        ...
        try:
            user=User.query.filter(User.name==uname,User.password==passd).one()
        except:
...
def signup():
    ...
    if request.method=='POST':
        ...
        if uname not in (i.name for i in User.query.all()):
            user=User(name=uname,password=passd)
            db.session.add(user)
            db.session.commit()
            return login()
...
def add():
    ...
    if request.method=='POST':
        ...
        member=Member(name=mname,age=mage,total_expense=0,limit=mlimit,user_id=current_user.get_id())
...
def update_member(id):
    ...
    if request.method=='POST':
        try:
            if request.form.get('mem_name')!=t.name or request.form.get('mem_age')!=t.age :
                db.session.query(Expense).filter(Expense.id==id).delete()
            updict={Member.name:request.form['mem_name'],Member.age:request.form['mem_age'], Member.limit:request.form['mem_limit']}
            print(updict)
...
def view(id):
    ...
    for i in me:
        if i.created_date.strftime(comp)>=llim and i.created_date.strftime(comp)<=hlim:
...
def add_expense(id):
    ...
    if request.method=='POST':
        try:
            ...
            created=datetime.strptime(request.form.get("time"),'%d/%b/%Y, %H:%M:%S.%f')
    ...
    return render_template('add_expense.html',member=Member.query.get(id),datetime=datetime)
